...
typedef google::dense_hash_map<const char*, HashValue, SuperFastHashString, EqualString> gg_str_hashmap;
...
IosHash *ios_hash_new() {
    ...
    _this->root.set_deleted_key(DELETED_KEY);
	return _this;
}
...
void ios_hash_free(IosHash *_this) {
    ...
    while (it != _this->root.end()) {
        free((void*)(*it).first);
        if ((*it).second.type == HASH_TYPE_STRING)
            free((void*)(*it).second.value.str);
        ++ it;
    }
	delete _this;
}
...
void ios_hash_put(IosHash *_this, const char *key, HashValue value) {
    ...
    // ios_hash_remove (_this, key);
    if (value.type == HASH_TYPE_STRING)
        value.value.str = strdup(value.value.str);
...
void ios_hash_foreach(IosHash *_this, HashMapAction *action)
{
    gg_str_hashmap::iterator it = _this->root.begin();
    while (it != _this->root.end()) {
        action->action((*it).first, &((*it).second));
        ++ it;
    }
}
...
HashMap::HashMap(const HashMap &h) {
    hash = ios_hash_new();
    gg_str_hashmap::iterator it = h.hash->root.begin();
    while (it != h.hash->root.end()) {
...
HashMap& HashMap::operator=(const HashMap& hmap)
{
    ios_hash_free(hash);
    hash = ios_hash_new();
    gg_str_hashmap::iterator it = hmap.hash->root.begin();
    while (it != hmap.hash->root.end()) {
        const char *key   = (*it).first;
        HashValue  &value = (*it).second;
        put(key,value);
        ++ it;
