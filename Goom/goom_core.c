...
#define nKaleidoFold 11
static const int kaleidoFoldCounts[nKaleidoFold] =
    { 2, 3, 4, 5, 6, 7, 8, 13, 15, 21, 25 };
...
static PluginParameters *zoomFilterParams (PluginInfo *goomInfo) {
	int g;
	for (g = 0; g < goomInfo->nbParams; g++)
		if (strcmp (goomInfo->params[g].name, "Zoom Filter") == 0)
			return &goomInfo->params[g];
	return NULL;
}
        ...
        if (goomInfo->update.lockvar == 0) {
            ...
            if (goomInfo->sound.timeSinceLastGoom == 0) {
                ...
                if ((goomInfo->curGState->drawIFS) && (goomInfo->update.ifs_incr<=0)) {
                    ...
                    ifs_visualfx_reset(&goomInfo->ifs_fx, goomInfo);
                }
...
void update_message (PluginInfo *goomInfo, char *message) {
    ...
    if (message) {
        int i=1,j=0;
        strncpy (goomInfo->update_message.message, message, sizeof(goomInfo->update_message.message) - 1);
        goomInfo->update_message.message[sizeof(goomInfo->update_message.message) - 1] = 0;
    ...
    if (goomInfo->update_message.affiche) {
        ...
        msg[goomInfo->update_message.longueur] = 0;
