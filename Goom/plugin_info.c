...
#endif /* CPU_POWERPC */
/* Dispatch table: C fallbacks, NEON on aarch64, AltiVec on PPC.
   The historical x86 MMX/XMMX kernels (zoom_filter_mmx/xmmx,
   draw_line_mmx) were removed with the 2k26 trim — dead since HAVE_MMX
   was never defined in any build. An x86 SIMD port would add a new
   kernel + branch here instead of resurrecting mmx.c/xmmx.c. */
...
/*    p->methods.create_output_with_brightness = create_output_with_brightness;*/
    ...
    p->methods.zoom_filter = zoom_filter_neon;
#endif
