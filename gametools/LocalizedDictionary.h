...
public:
    ...
    ~LocalizedDictionary();
    const char *getName() const { return stdName.c_str(); }
private:
    void * dictionary;
	ios_fc::String stdName;
    const DataPathManager &datapathManager;
};

void Locales_SetDefault(const char *defaultLocale = NULL);
void Locales_Init(const char *forceLocale = NULL);
