...
namespace gametools {
    ...
    IosFontRef    GameUIDefaults::FONT             = NULL;
    IosFontRef    GameUIDefaults::FONT_TEXT        = NULL;
    IosFontRef    GameUIDefaults::FONT_INACTIVE    = NULL;
    IosFontRef    GameUIDefaults::FONT_SMALL_ACTIVE= NULL;
    IosFontRef    GameUIDefaults::FONT_SMALL_INFO  = NULL;
    IosFontRef    GameUIDefaults::FONT_FUNNY       = NULL;
    ...
    Widget::Widget(WidgetContainer *parent)
        ...
        position(0,0,0), m_isDead(false), hidden(false), focus(false), focusable(false), _drawRequested(true), receiveUp(false), receiveAll(false), m_hover(false)
    {
    ...
    void WidgetContainer::resize(int width, int height)
    {
        for (int i = 0; i < getNumberOfChilds(); ++i)
            getChild(i)->resize(width, height);
    }
	...
	void WidgetContainer::isHover(bool o) {
		Widget::isHover(o);
		if (o == false)
			for (int i = 0; i < childs.size() ; i++)
				childs[i]->isHover(false);
	}
    ...
    void Box::eventOccured(GameControlEvent *event)
    {
        ...
        else if (event->cursorEvent == kJoystickAxis || event->cursorEvent == kJoystickButton) {
            for (int i = 0 ; i < this->getNumberOfChilds() ; i++) {
                Widget *wid = this->getChild(i);
                if (wid->receiveAllEvents() || wid->haveFocus())
                    wid->eventOccured(event);
            }
        }
    ...
    void Box::handleMouseFocus(GameControlEvent *event)
    {
		...
		for (int i = 0 ; i < this->getNumberOfChilds() ; i++) {
            ...
            if (wid->receiveAllEvents())
                wid->eventOccured(event);
		}
		if (focusedWidget != NULL) {
			focusedWidget->giveFocus();
            if (!focusedWidget->receiveAllEvents())
    ...
    {
        std::auto_ptr<GameControlEvent> ev(GameUIDefaults::GAME_LOOP->getEventManager()->createGameControlEvent());
	...
	void AbsoluteContainer::arrangeWidgets() {
		int numChilds = getNumberOfChilds();
		for (int i = 0; i < numChilds; ++i) {
			Widget *w = getChild(i);
			ZBox::setWidgetPosition(w, getPosition() + getWidgetBackupPosition(w));
		}
	}
    ...
    SliderContainer::SliderContainer(GameLoop *loop)
        ...
        , m_slideSide(SLIDE_FROM_RIGHT), slidingTime(0.45)
    ...
    {
        if ( !(sliding) ) ZBox::eventOccured(event);
    }
    ...
    void SliderContainer::transitionToContent(Widget *content)
    {
        ...
        if (contentWidget != NULL) {
            switch (m_slideSide) {
                ...
                case SLIDE_FROM_BOTTOM:
                    m_outsidePosition = getSize().y;
                    break;
				case SLIDE_FROM_LEFT_ONE_SHOT:
					add(contentWidget);
                    m_outsidePosition = getSize().x;
					setWidgetPosition(contentWidget, -m_outsidePosition);
                    break;
				case SLIDE_FROM_RIGHT_ONE_SHOT:
					add(contentWidget);
                    m_outsidePosition = -getSize().x;
					setWidgetPosition(contentWidget, -m_outsidePosition);
                    break;
        ...
        else m_outsidePosition = getSize().x;
    }
    ...
    void SliderContainer::idle(double currentTime)
    {
        ...
        if (t > slidingTime) // At end of sliding period
        {
            if (slideout) // If the previous widget has gone
            {
				...
				if ((m_slideSide == SLIDE_FROM_LEFT_ONE_SHOT) || (m_slideSide == SLIDE_FROM_RIGHT_ONE_SHOT || m_slideSide == SLIDE_CUSTOM)) {
					slideout = false;
					sliding = false;

					suspendLayout();
					setPosition(backupedPosition);
					setWidgetPosition(contentWidget, 0);
					resumeLayout();
					requestDraw();
                ...
                else if (contentWidget != NULL) // there is a new widget to show
                {
		...
		if ((m_slideSide == SLIDE_FROM_LEFT)
			...
			|| (m_slideSide == SLIDE_FROM_LEFT_ONE_SHOT)
			|| (m_slideSide == SLIDE_FROM_RIGHT_ONE_SHOT))
        ...
        {
            double stime = fabs(t);
            double shtime = slidingTime;
			if ((m_slideSide == SLIDE_FROM_LEFT)
				|| (m_slideSide == SLIDE_FROM_RIGHT)
				|| (m_slideSide == SLIDE_FROM_LEFT_ONE_SHOT)
				|| (m_slideSide == SLIDE_FROM_RIGHT_ONE_SHOT))
				pos.x += distance*sin(1.57079632679489661922*stime/shtime);
			else
				pos.y += distance*sin(1.57079632679489661922*stime/shtime);
			
        }
        else
        {
            double stime = fabs(slidingTime-t);
            double shtime = slidingTime;
			if ((m_slideSide == SLIDE_FROM_LEFT)
				|| (m_slideSide ==SLIDE_FROM_RIGHT)
				|| (m_slideSide == SLIDE_FROM_LEFT_ONE_SHOT)
    ...
    void SliderContainer::addContentWidget()
    {
      ...
      sliding = s;
      add(contentWidget);
      Vec3 pos = Vec3(getSize().x, backupedPosition.y);
      ZBox::setPosition(pos);
...
#define kHScrollListNbAddedItemsToEachSide 1
    ...
    Text::Text()
        : label(""), offset(0.0,0.0,0.0), m_textAlign(TEXT_LEFT_ALIGN),
		  m_autoSize(true), m_slideSound(GameUIDefaults::SLIDE_SOUND), m_shadow(false), mdontMove(true)
    {
    ...
    Text::Text(const String &label, IosFontRef font, bool autosize)
    ...
    {
        if (font.get() == NULL) this->font = GameUIDefaults::FONT_TEXT;
        setPreferedSize(Vec3(m_autoSize?this->font->getTextWidth(label):0.0f, this->font->getHeight(), 1.0));
        moving = false;
        startMoving = false;
    }
    ...
    // Image
    //
	Image::Image() : m_flip(IMAGE_NOFLIP), m_image(NULL), m_focusedImage(NULL), m_hoverImage(NULL), m_invertFocusMode(false)
    {
		m_red = m_green = m_blue = 1.0f;
    }

    Image::Image(const AnisoSurface &image, ImageAlign align, float scale, bool useGlobalScaleFactor)
	  : m_flip(IMAGE_NOFLIP), m_image(NULL), m_focusedImage(NULL), m_hoverImage(NULL), m_invertFocusMode(false), m_align(align)
    {
        setImage(image, scale, useGlobalScaleFactor);
		m_red = m_green = m_blue = 1.0f;
    }
    ...
    void Image::setImage(const AnisoSurface &image, float scale, bool useGlobalScaleFactor)
    {
		if (image.get() == m_image.get() && scale == m_scale) return;
		m_scale = scale;
        m_image = image;
		...
		m_useGlobalScaleFactor = useGlobalScaleFactor;
		if (image.get() != NULL) {
			if (useGlobalScaleFactor) {
			this->setPreferedSize(Vec3(m_image.width() * GTGetScaleFactor() * scale, m_image.height() * GTGetScaleFactor() * scale));
			this->setSize(Vec3(m_image.width() * GTGetScaleFactor() * scale, m_image.height() * GTGetScaleFactor() * scale));
			}
			else {
			this->setPreferedSize(Vec3(m_image.width() * scale, m_image.height() * scale));
			this->setSize(Vec3(m_image.width() * scale, m_image.height() * scale));
			}
		}
    }
    ...
    {
        IosRect dstRect;
        IosSurface *imageToDraw = m_image.get();
		if (imageToDraw == NULL) return;
        Vec3 pos = this->getPosition();
        Vec3 size = this->getSize();
		float gtscale = (m_useGlobalScaleFactor ? GTGetScaleFactor() : 1.0f );
        switch (m_align) {
            case IMAGE_CENTERED:
                dstRect.x = pos.x - (m_image.width() * gtscale * m_scale - size.x)/2;
                dstRect.y = pos.y - (m_image.height() * gtscale * m_scale - size.y)/2;
                dstRect.h = m_image.height() * gtscale * m_scale;
				dstRect.w = m_image.width() * gtscale * m_scale;
                break;
            default:
                dstRect.x = pos.x;
                dstRect.y = pos.y;
                dstRect.h = size.y; // imageToDraw->h * m_scale * GTGetScaleFactor();
				dstRect.w = size.x; // imageToDraw->w * m_scale * GTGetScaleFactor();
        }
        if (haveFocus() ^ m_invertFocusMode) {
			...
			*/
            if (!m_focusedImage.isNull())
				imageToDraw = m_focusedImage.get();
        }
		if (isHover() && !m_hoverImage.isNull()) imageToDraw = m_hoverImage.get();
		switch (m_flip) {
			case IMAGE_NOFLIP: break;
			case IMAGE_XFLIP:
				dstRect.x +=  dstRect.w;
				dstRect.w = - dstRect.w;
				break;
		...
		if (m_red + m_green + m_blue < 2.9f)
			dt->nextDrawColor(m_red, m_green, m_blue);
    ...
    void Button::init(IosFontRef fontActive, IosFontRef fontInactive)
    {
        if (fontInactive.get() == NULL) fontInactive = GameUIDefaults::FONT_INACTIVE;
        if (fontActive.get() == NULL)   fontActive = GameUIDefaults::FONT;
    ...
    Button::Button(const String &label, IosFontRef fontActive, IosFontRef fontInactive)
    ...
    void EditField::init(IosFontRef fontActive, IosFontRef fontInactive)
    {
    ...
    EditField::~EditField() {
        if (haveFocus())
            gtNotifier.notify("HideKeyboard", this);
    }
    ...
    void EditField::idle(double currentTime)
    {
        ...
        if (repeat) {
            double t = gtCommander->getTimeMs() - repeat_date;
            if (t > repeat_speed) {
                ...
                repeat_date = gtCommander->getTimeMs();
                repeat = true;
                // repeatEvent.reset(e->clone());
            }
        }
    }
    ...
    void EditField::giveFocus() {
        if (!haveFocus()) {
            boing();
            gtNotifier.notify("ShowKeyboard", this);
        }
        Text::giveFocus();
        font = fontActive;
        if (editOnFocus) {
    ...
    void ControlInputWidget::init(IosFontRef fontActive, IosFontRef fontInactive)
    {
        if (fontInactive.get() == NULL) fontInactive = GameUIDefaults::FONT_SMALL_INFO;
        if (fontActive.get() == NULL)   fontActive = GameUIDefaults::FONT_SMALL_ACTIVE;
