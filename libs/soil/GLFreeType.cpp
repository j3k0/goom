...
using std::string;
    ...
    GlyphData::~GlyphData() {
        if (texture) {
            glDeleteTextures(1, &texture); GL_GET_ERROR();
        }
    }
    ...
    void make_dlist ( FT_Face face, unsigned short ch, float h, GlyphData &glyphData) {
        ...
        float fx = (float)bitmap_glyph->left;
        float fy = (float)h - (float)bitmap_glyph->top;
    ...
    void GLFont::init(void *data, int dataSize, float h, float letter_spacing, float line_spacing) {
        m_data = data;
        m_dataSize = dataSize;
        ...
        if (FT_New_Memory_Face(library, (FT_Byte *)m_data, m_dataSize, 0, &face ))
        ...
        FT_Set_Char_Size( face, h * 64.0f, h * 64.0f, 96, 96);
    }
	...
	float GLFont::getWidthUnicode(float size, const unsigned short *text)  {
        if (text == NULL)
            return 0.;
        static MultiLine *lines = NULL;
        if (lines == NULL) lines = new MultiLine();
        textToLines(text, *lines);
		
        float width = 0.0f;
        for(unsigned int i=0;i<lines->size();i++) {
            float w = 0.0f;
            for (unsigned int j=0;j<lines->lines[i].size();++j) {
                w += getGlyphData((unsigned short)lines->lines[i].line[j]).letter_width;
            }
            if (w > width) width = w;
        }
		return width * size / h;
    ...
    float GLFont::getHeightUnicode(float size, const unsigned short *text)  {
        return size;
    }

    void GLFont::printCenteredUnicode(float size, float x, float y, const unsigned short *text, float dx, float dy)  {
        if (text == NULL)
            return;
        MultiLine lines;
        textToLines(text, lines);
        ...
        // offset x,y
        // print
        printUnicode(size, x - width/2.0, y, text,'l', dx, dy);
    }

    void GLFont::printUnicode(float size, float x, float y, const unsigned short *text, char align, float dx, float dy) {
        // TODO: Manage dx, dy
        if (text == NULL)
            return;
        ...
        glPushMatrix();
        glTranslatef(x,y,0);
        if (coef < 0.99f || coef > 1.01f)
            glScalef(coef,coef,coef);
        
        float lineH      = line_spacing;
        float emptyLineH = line_spacing * 0.5f;
        float totalY = 0.0f;
         
        for(unsigned int i=0;i<lines.size();i++) {
            glPushMatrix();
            const Line &line = lines.lines[i];

            float lh = lineH;
            if ((line.size() == 0) || ( (line.size() == 1) && (line.line[0] == ' ') )) {
                lh = emptyLineH;
            }
			...
			if (align == 'r') {
				...
				for (unsigned int j=0;j<line.size();++j) {
                    const GlyphData & gd = getGlyphData((unsigned short)line.line[j]);
					w += gd.letter_width;
				}
				glTranslatef(-w,totalY,0);
			}
			else {
				glTranslatef( 0,totalY,0);
			}
            //FT_UInt previous_index = 0;
            for (unsigned int j=0; j<line.size(); ++j) {
                unsigned short ch = line.line[j];
                ...
                glTranslatef(dx*glyphData.letter_width,dy*glyphData.letter_width,0);
            }
            glPopMatrix();
            totalY += lh;
        }
    ...
    void GLFontLibrary::unrefGlObjects() {
        GLFontList::iterator it = m_fontList.begin();
        while (it != m_fontList.end()) {
            (*it)->unrefGlObjects();
            ++it;
        }
    }
    void GLFontLibrary::freeGlObjects() {
        GLFontList::iterator it = m_fontList.begin();
        while (it != m_fontList.end()) {
            (*it)->freeGlObjects();
            ++it;
