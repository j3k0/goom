	...
	)
{
	...
	*/
    ...
    for ( y = 0; y < resampled_height; ++y )
    {
        ...
        int fp_sampley = y * fp_dy;
    	int inty = fp_sampley >> 8;
		if (inty > height - 1) {
            inty = height - 1;
        }
        ...
        {
			int base_index;
            int fp_samplex = x * fp_dx;
			int intx = fp_samplex >> 8;
            ...
            for (c = 0; c < channels; ++c) {
            	resampled[out_index] = (unsigned char)orig[base_index];
                ++out_index;
				++base_index;
            }
        }
    ...
    for ( y = 0; y < resampled_height; ++y )
    {
        ...
        for (x = 0; x < resampled_width; ++x)
        {
            ...
            for (c = 0; c < channels; ++c) {
            	/*	do the sampling	*/
				int fp2_value = 0x8000;
				fp2_value += orig[base_index]
							* (0x100 - fp_samplex) * (0x100 - fp_sampley);
				fp2_value += orig[base_index+channels]
							* (fp_samplex) * (0x100 - fp_sampley);
				fp2_value += orig[base_index+width*channels]
							* (0x100 - fp_samplex) * (fp_sampley);
				fp2_value += orig[base_index+width*channels+channels]
							* (fp_samplex) * (fp_sampley);
				/*	move to the next channel	*/
				++base_index;
            	/*	save the new value	*/
            	resampled[out_index] =
						(unsigned char)(fp2_value >> 16);
                ++out_index;
            }
        }
    }
