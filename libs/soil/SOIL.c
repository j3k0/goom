	...
	)
{
    ...
    if (texture_check_size_enum == GL_MAX_TEXTURE_SIZE)
	...
	if( tex_id )
	{
		...
		case 3:
            ...
            if ( flags & SOIL_FLAG_TEXTURE_16BITS )
                internal_texture_format = GL_UNSIGNED_SHORT_5_6_5;
            else
                internal_texture_format = original_texture_format;
			break;
		case 4:
			original_texture_format = GL_RGBA;
            if ( flags & SOIL_FLAG_TEXTURE_16BITS_4444 )
                internal_texture_format = GL_UNSIGNED_SHORT_4_4_4_4;
            else if ( flags & SOIL_FLAG_TEXTURE_16BITS_5551 )
                internal_texture_format = GL_UNSIGNED_SHORT_5_5_5_1;
            else if ( flags & SOIL_FLAG_TEXTURE_16BITS )
                internal_texture_format = GL_UNSIGNED_SHORT_4_4_4_4;
            else
                internal_texture_format = original_texture_format;
			break;
		...
		} else if( ( flags & SOIL_FLAG_TEXTURE_16BITS ) && ( original_texture_format != internal_texture_format ) )
        {
            int i;
            GLsizei imageSize = 0;
            if( internal_texture_format == GL_UNSIGNED_SHORT_4_4_4_4 )
            {
                for( i = 0; i < width*height; i += 1 )
                {
                    ...
                    uint8_t b = img[4*i+2];
                    uint8_t a = img[4*i+3];
                    img[2*i+1] = ( r & 0xf0 ) | ( (g >> 4) & 0x0f );
                    img[2*i+0] = ( b & 0xf0 ) | ( (a >> 4) & 0x0f );
                }
                imageSize = width*height*2;
                glTexImage2D( opengl_texture_target, 0,
                              GL_RGBA, width, height, 0,
                              GL_RGBA, GL_UNSIGNED_SHORT_4_4_4_4, img );
                check_for_GL_errors( "glTexImage2D" );
            } else if( internal_texture_format == GL_UNSIGNED_SHORT_5_6_5 )
            {
                for( i = 0; i < width*height; i += 1 )
                {
                    uint8_t r = img[3*i+0];
                    uint8_t g = img[3*i+1];
                    uint8_t b = img[3*i+2];
                    img[2*i+0] = ( ( g << 3 ) & 0xe0 ) | ( (b >> 3) & 0x1f );
                    img[2*i+1] = ( r & 0xf8 ) | ( (g >> 5) & 0x07 );
                }
                imageSize = width*height*2;
                glTexImage2D( opengl_texture_target, 0,
                              GL_RGB, width, height, 0,
                              GL_RGB, GL_UNSIGNED_SHORT_5_6_5, img );
                check_for_GL_errors( "glTexImage2D" );
            } else if( internal_texture_format == GL_UNSIGNED_SHORT_5_5_5_1 )
            {
                for( i = 0; i < width*height; i += 1 )
                {
                    uint8_t r = img[4*i+0];
                    ...
                    uint8_t a = img[4*i+3];
                    img[2*i+1] = ( r & 0xf8 ) | ( (g >> 5) & 0x07 );
                    img[2*i+0] = ( ( g << 3 ) & 0xc0 ) | ( (b >> 2) & 0x3e ) | ( (a >> 7) & 0x01 );
                }
                imageSize = width*height*2;
                glTexImage2D( opengl_texture_target, 0,
                              GL_RGBA, width, height, 0,
                              GL_RGBA, GL_UNSIGNED_SHORT_5_5_5_1, img );
                check_for_GL_errors( "glTexImage2D" );
            } else
            {
                printf("Failed to generate 16 bits texture\n");
                glTexImage2D(
                    opengl_texture_target, 0,
                    internal_texture_format, width, height, 0,
                    original_texture_format, GL_UNSIGNED_BYTE, img );
